...
import guesthouse.exeption.ReservationError;
...
public class ReservationRepositoryImpl implements ReservationRepository {
	...
	public void addReservation(Reservation newReservation) {
		...
		System.out.println(
				newReservation.getDataReservation() + newReservation.getDataStart() + newReservation.getDataStop()
						+ newReservation.getId() + newReservation.getIdClient() + newReservation.getIdRoom());
		if (list.isEmpty()) {
			session.save(newReservation);
		}
		for (Reservation r : list) {
			if (r.getIdRoom() == newReservation.getIdRoom() || dateTimeStart.isAfter(dateTimeStop)) {
				throw new ReservationError(newReservation.getIdRoom(), newReservation.getDataStart(),
						newReservation.getDataStop());
			} else {
				session.save(newReservation);
			}
